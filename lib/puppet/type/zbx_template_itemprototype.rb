Puppet::Type.newtype(:zbx_template_itemprototype) do

  desc 'zbx_template_itemprototype is used to manage the item prototype contained within a Zabbix Template discovery rule'
  ...
  newproperty(:ruleid) do
    desc <<-EOT
      (read-only) ID of the parent discovery rule.
    EOT
  end

  newproperty(:discovery_rule) do
    desc <<-EOT
      Name of the parent discovery rule.
    EOT
  end
